...
ЯЗЫК ОТВЕТА: квест ниже передан на определённом языке интерфейса — всегда пиши поле "reason" на ТОМ ЖЕ языке, что и переданные тебе "Квест"/"Сценарий"/"Задание ученику", независимо от языка, на котором ученик написал свой ответ.`;

function buildUserPrompt(context: ChallengeContext, answer: string): string {
  const language = context.locale === "ru" ? "Russian" : "English";
  return `[Write the "reason" field in ${language}]\nКвест: ${context.questTitle}\nСценарий: ${context.scenario}\nЗадание ученику: ${context.actionPrompt}\n\nОтвет ученика:\n${answer}`;
}
